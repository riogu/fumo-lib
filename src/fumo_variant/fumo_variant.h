...
#define fumo$is_same_t(X, Y) (({\
        ...
        if (is_x_v && is_y_v) {\
            is_same_t = ( (*(fumo$variant*)&X).type_id == (*(fumo$variant*)&Y).type_id );\
        }\
        if (is_x_v && !is_y_v) {\
            is_same_t = ( (*(fumo$variant*)&X).type_id == fumo$get_type_id(typeof(Y)) );\
        }\
        if (!is_x_v && is_y_v) {\
            is_same_t = ( fumo$get_type_id(typeof(X)) == (*(fumo$variant*)&Y).type_id );\
        }\
        if (!is_x_v && !is_y_v) {\
            is_same_t = ( fumo$get_type_id(typeof(X)) == fumo$get_type_id(typeof(Y)) );\
        }\
        is_same_t;\
}))
