...
typedef struct Result {
    union _Result_value_t {
        Result_Position_int _Result_Position_int;
        Result_int_str _Result_int_str;
    } _value;
} Result;

#define add_result_t(...)
add_result_t(int, str)
add_result_t(str, Position)
add_result_t(Rectangle, Position)

#define Ok(_v) \
    (typeof(*(Result*)0)) {._value = (union _Result_value_t) {{._Ok = _v}}};
#define Err(_v) \
    (typeof(*(Result*)0)) {._value = (union _Result_value_t) {{._Err = _v}}};
